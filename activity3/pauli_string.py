...
class PauliString(object):
    ...
    def from_zx_bits(cls, zx_bits):
        ...
        """
        halflen = int(len(zx_bits)/2)
        z_bits = np.zeros(halflen)
        x_bits = np.zeros(halflen)

        for i in range(len(zx_bits)):
            if i < halflen:
                z_bits[i] = zx_bits[i]
            else:
                x_bits[i-halflen] = zx_bits[i]      
    ...
    def from_str(cls, pauli_str):
        ...
        z_bits = np.zeros(len(pauli_str))
        x_bits = np.zeros(len(pauli_str))

        for c in range(len(pauli_str)):
            if pauli_str[c] == 'X':
                z_bits[c] = 1
                x_bits[c] = 0
            elif pauli_str[c] == 'Z':
    ...
    def to_zx_bits(self):
        ...
        """
        zx_bits = np.array(np.append(self.z_bits,self.x_bits),dtype=bool)
    ...
    def to_xz_bits(self):
        ...
        xz_bits = np.array(np.append(self.x_bits,self.z_bits),dtype=bool)
    ...
    def ids(self):
        ...
        """
        ids = np.zeros(len(self.x_bits), dtype=bool)

        for i in range(len(self.x_bits)):
            if self.x_bits[i] and self.z_bits[i]:
                ids[i] = True
            else:
                ids[i] = False
